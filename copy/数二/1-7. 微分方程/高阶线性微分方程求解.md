...
1. 当 $f(x) = P_{n}(x) e^{ax}$ 时，特解需要设为： $$y^* = e^{ax}Q_{n}(x) x^{k}$$ ![[../assets/diff_eq_special_1.png]]
 2. 当 $f(x) = e^{ax}[P_{m}(x)\cos \beta x +P_{n}(x)\sin \beta x]x^{k}$ 时，特解需要设为： $$y^* = e^{ax}[Q_{l}^{(1)}(x)\cos \beta x + Q_{l}^{(2)}(x) \sin \beta x] x^{k}$$ ![[../assets/diff_eq_special_2.png]] 还可以用 *微分算子法* 求解，具体解法见书 `p275`。
